...
public class BookingConsolidatedServiceImpl implements IBookingConsolidatedService {
    ...
    @Autowired
    private com.ruoyi.system.mapper.BlPartiesMapper blPartiesMapper;

    @Autowired
    private com.ruoyi.system.mapper.BlCargoMapper blCargoMapper;

    @Autowired
    private com.ruoyi.system.mapper.BlFreightMapper blFreightMapper;
    ...
    public BookingConsolidated directProcessAndSave(String filePath, Map<String, Object> editedData) {
        ...
        log.info("准备插入bill_of_lading_v5表(新分表结构)，bl_no: {}, booking_no: {}, 文件路径: {}",
                bl.getBlNo(), bl.getBookingNo(), filePath);
        billOfLadingMapper.insertBillOfLading(bl);
        billOfLadingMapper.insertBlParties(bl);
        billOfLadingMapper.insertBlCargo(bl);
        billOfLadingMapper.insertBlFreight(bl);
    ...
    public BookingConsolidated generateAndSavePdf(BookingConsolidatedDto userDto) {
        ...
        log.info("准备插入bill_of_lading相关表(MP全面应用)，bl_no: {}, booking_no: {}, 文件路径: {}",
                bl.getBlNo(), bl.getBookingNo(), newPdfPath);
        
        // 1. 插入主表 (自动回填主键 id)
        billOfLadingMapper.insert(bl);
        
        // 2. 设置关联 ID 并插入子表
        Long blId = bl.getId();
        
        bl.getParties().setBlId(blId);
        blPartiesMapper.insert(bl.getParties());
        
        bl.getCargo().setBlId(blId);
        blCargoMapper.insert(bl.getCargo());
        
        bl.getFreight().setBlId(blId);
        blFreightMapper.insert(bl.getFreight());
    ...
    public BookingConsolidated saveToDbOnly(BookingConsolidatedDto userDto) {
        ...
        log.info("准备插入bill_of_lading相关表(MP全面应用)，bl_no: {}, booking_no: {}",
